export const ConfirmationAlert = ({
    title, subtitle, affirmativeText, handleAffermative, cancelText, handleCancel,
}) => (
    <div className="alert alert-warning alert-dismissible fade show mt-4" role="alert">
        <div><strong>{title}</strong></div>
        {subtitle && <div>{subtitle}</div>}
        <button type="button" className="btn btn-warning mt-3" onClick={handleAffermative}>{affirmativeText}</button>
